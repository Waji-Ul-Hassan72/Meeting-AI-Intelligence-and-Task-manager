...
const isManager = (req) => {
    return getUserRole(req) === "project manager";
};

const isMember = (req) => {
    return getUserRole(req) === "developer";
};
...
const getTaskCounts = async (
    ...
    if (projectId) {
        ...
        conditions.push(
            `project_id = $${params.length}::integer`
        );
    }
    ...
    try {
        ...
        if (isMember(req)) {
            ...
            if (
                ...
                return res.status(403).json({
                    error:
                        "Members cannot assign or reassign tasks.",
                });
            }
            ...
            if (
                ...
                String(body.project_id) !==
                    String(existingTask.project_id)
            ) {
                return res.status(403).json({
                    error:
                        "Members cannot move tasks between projects.",
                });
            }
        }
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (!isManager(req)) {
                return res.status(403).json({
                    error:
                        "Only managers can delete all project tasks.",
                });
            }
